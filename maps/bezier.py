...
def generate(cell):
	...
	possible = []
	if not cell.up:
		possible.append((random.randint(25, 35), 35))
		#possible.append((random.randint(10, 50), 35))
	if not cell.down:
		possible.append((random.randint(25, 35), 0))
		#possible.append((random.randint(10, 50), 0))
	if not cell.left:
		possible.append((0, random.randint(13, 23)))
		#possible.append((0, random.randint(8, 28)))
	if not cell.right:
		possible.append((59, random.randint(13, 23)))
		#possible.append((59, random.randint(8, 28)))
	
	if len(possible) == 0:
		return lvl

	paths = random.randint(2,10)

	used = set()

	for path in range(paths):
		if len(possible) == 1:
			pts = (possible[0], possible[0])
		else:
	...
	for path in follow_ups:
		...
		new = random.choice(all_paths)
		add_path(lvl, path, new)
